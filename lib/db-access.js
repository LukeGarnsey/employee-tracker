...
const updateEmployee = (id, roleID) => new Promise((resolve, reject)=>{
  db.query(updateEmployeeQuery, [roleID, id], (err, results)=>{
    if(err)
      reject(err);
    else
      resolve(results);
...
WHERE
r.department_id = ?`
const viewBudget = (deptID) => new Promise((resolve, reject)=>{
  db.query(budgetQuery, [deptID], (err, result)=>{
    if(err)
      reject(err);
    else
      resolve(result[0].salaries);
  });
});

const employeesByDeptQuery =`SELECT
concat(e1.first_name,' ',e1.last_name) AS Employee_Name,
r1.title AS Title, r1.salary as Salary,
concat(e2.first_name,' ',e2.last_name) AS Manager_Name
FROM
employee e1
JOIN
...
WHERE
r1.department_id = ?`;
const viewDepartmentEmployees = (deptID) => new Promise((resolve, reject)=>{
  db.query(employeesByDeptQuery, [deptID], (err, result) =>{
    if(err)
      reject(err);
    else
    {
...
module.exports = {viewAllDepartments, viewAllRoles, viewAllEmployees,
  ...
  updateEmployee, viewBudget, viewDepartmentEmployees};